...
class Hier8_net():
    ...
    def hier8_net(self,A,k): 
        ...
        for i in range(0,k):
            ...
            if(i == 0):
                ...
                leaves = np.where(is_leaf==1)
                temp_priority = priorities[leaves]
                if(np.size(temp_priority==0)):
                    continue;
                ...
                new_nodes = np.array([result_used+1, result_used+2])
            ...
            clusters.append(np.where(cluster_subset == 0))
            clusters.append(np.where(cluster_subset == 1))
            Ws.append(W[:,0])
            Ws.append(W[:,1])
            splits[i] = split_node
            is_leaf[new_nodes] = 1 
    ...
    def  trial_split(trial_allowance, unbalance, min_priority, A, subset, W_parent):
    ...
    def actual_split(A, subset, W_parent):
        ...
        if( len(subset) <= 3):
            ...
            term_subset = np.where(np.sum(A[:,2]) !=0)

        return ;
                


    def nmfsh_comb_rank2(self,A, W, H):
